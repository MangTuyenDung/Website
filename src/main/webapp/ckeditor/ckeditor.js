...
for(v in u)x.elements[v]=r();var y={
    ...
    elements:{$:function(T){var U=T.attributes;if(U){if(U['data-cke-temp'])return false;var V=['name','href','src'],W;for(var X=0;X<V.length;X++){W='data-cke-saved-'+V[X];W in U&&delete U[V[X]];}}return T;},
        table:function(T){var U=T.children;U.sort(function(V,W){return V.type==1&&W.type==V.type?e.indexOf(t,V.name)>e.indexOf(t,W.name)?1:-1:0;
        });
    },
    embed:function(T){
        var U=T.parent;
...
function A(E){
    return function(F){
        ...
        if(J&&(J.name&&J.name=='br'||J.value&&(K=J.value.match(y)))){
            ...
            else if(L.name=='br')G.splice(H-1,1);else L.value=L.value.replace(y,'');
        }
    };
...
for(var C in x.$listItem)B.elements[C]=A();var D={
    elements:{}
};

for(C in x.$listItem)D.elements[C]=A(true);j.add('list',{
    init:function(E){
...
(function(){
    ...
    j.add('pastefromword',{
        ...
        n.on('paste',function(q){
            ...
            ,requires:['clipboard']});})();(function(){var m={exec:function(n){var o=e.tryThese(function(){var p=window.clipboardData.getData('Text');if(!p)throw 0;return p;});if(!o){n.openDialog('pastetext');
            return false;
        }else n.fire('paste',{
            text:o
        });
...
function t(T){
    ...
    if(ai&&af&&!af.collapsed){
        var aq=J(ax,U),ar=aq.hasAttributes(),as=af.getCommonAncestor(),at={
            styles:{},
            attrs:{},
            blockedStyles:{},
            blockedAttrs:{}
...
for(au in at.attrs)aq.removeAttribute(au);for(av in at.styles)aq.removeStyle(av);if(ar&&!aq.hasAttributes())aq=null;
if(aq){
...
function w(T){
    ...
    if(Y)for(var aa in Y)V.removeAttribute(aa,Y[aa]);if(X.styles)for(var ab in X.styles){
